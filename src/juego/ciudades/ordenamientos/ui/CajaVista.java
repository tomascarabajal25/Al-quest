...
public class CajaVista extends ObjetoVista {
    ...
    // ── Sprite compartido entre todas las instancias ──────────────────────────
    private static BufferedImage spriteCaja = cargarSprite();
    ...
    public CajaVista(Caja caja, int worldX, int worldY, int indiceLogico) {
        super(worldX, worldY, caja.getNombre(), false, spriteCaja);
        this.caja         = caja;
        this.indiceLogico = indiceLogico;
    }

    /**
     * Carga el sprite de caja una sola vez para todas las instancias.
     * Si no se encuentra, devuelve null y se usará el dibujo vectorial como fallback.
     */
    private static BufferedImage cargarSprite() {
        try {
            return ImageIO.read(
                CajaVista.class.getResourceAsStream(
                    ConfiguracionDeOrdenamientos.SPRITE_CAJA));
        } catch (IOException | IllegalArgumentException e) {
            System.out.println("Advertencia: no se pudo cargar sprite de caja: "
                + ConfiguracionDeOrdenamientos.SPRITE_CAJA);
            return null;
        }
    }
    ...
    private void dibujarCajaVectorial(Graphics2D g2, int sx, int y, int tam, int alto) {
        java.awt.GradientPaint grad = new java.awt.GradientPaint(
            sx, y, COLOR_NORMAL_TOP, sx, y + alto, COLOR_NORMAL_BOT);
        g2.setPaint(grad);
    ...
    private void dibujarEtiqueta(Graphics2D g2, int sx, int sy, int tam) {
        ...
        g2.drawString(linea1, sx + (tam - fm.stringWidth(linea1)) / 2, sy + tam - 14);

        String linea2 = String.valueOf(caja.getTamanio());
        g2.drawString(linea2, sx + (tam - fm.stringWidth(linea2)) / 2, sy + tam - 4);
    }
    ...
    private Color obtenerColorBorde() {
        ...
        if (seleccionada) {
            // parpadeo: visible PARPADEO_VISIBLE_TICKS, invisible el resto del ciclo
            return (tickParpadeo % ConfiguracionDeOrdenamientos.PARPADEO_CICLO_TICKS
                        < ConfiguracionDeOrdenamientos.PARPADEO_VISIBLE_TICKS)
                ? COLOR_SELECCIONADA
                : null;
        }
        return null;
