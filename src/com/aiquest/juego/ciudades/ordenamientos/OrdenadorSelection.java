...
public class OrdenadorSelection<T extends Comparable<T>> extends Ordenador<T> {
    ...
    public void ordenar(List<T> elementos, AdministradorDePasos<T> administradorPasos) {
        ...
        for (int i = 0; i < n - 1; i++) {
         ...
         // lo comento pq podria estar bien mostrar cada comparacion en el bitmap, pero se vuelve mucha cantidad de bitmaps q no son tan necesarios para el com.aiquest.com.aiquest.juego
            for (int j = i + 1; j < n; j++) {
                if (elementos.get(j).compareTo(elementos.get(indiceMinimo)) < 0) {
                    indiceMinimo = j;
